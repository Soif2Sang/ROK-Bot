...
class Tasks:
    ...
    def script_pause(self):
        try:
            ...
            if self.frame.stop or self.frame.end_tasks_button.cget("state") =="disabled":
    ...
    def enter_academy(self):
        self.click(uniform(740,765), uniform(250,270))
    ...
    @get_name
    def select_tech(self, compteur = 0):
        alliance_tech_logo = self.adb.find_img(target="alliance_tech", confidence=0.8)
        if alliance_tech_logo is not None:
            self.click(alliance_tech_logo[0] + uniform(0, 30), alliance_tech_logo[1] + uniform(0, 15))
            self.better_sleep((2, 3))
            ...
            if self.adb.find_img(target="get_more_rss") is not None:
                ...
                self.click(uniform(840, 950), uniform(128,150))
                self.better_sleep((1, 1.425))
            self.click(uniform(1106, 1125), uniform(62, 79))
            self.better_sleep((1, 1.425))
        else:
            print("not found")
    @get_name
    def enter_hut(self):
        x, y = uniform(682,693 ), uniform(284, 310)
        self.click(x, y)
        self.better_sleep((1.5, 2))
    ...
    def is_able_to_build(self):
        if self.adb.find_img(target='build'):
            return True
        else:
            return False
    ...
    def is_in_builder(self):
        if self.adb.find_img(target='builder'):
            return True
        else:
            return False
    @get_name
    ...
    @get_name
    def select_city_hall(self):
        self.click(uniform(510,540), uniform(280,300))
        self.better_sleep((0.9, 1.2))
    @get_name
    ...
    def setup_view(self):
        ...
        self.better_sleep((0.9, 1.5))
    @get_name
    def upgrade_city_hall(self):
        self.setup_view()
        self.better_sleep((0.9, 1.2))
        self.select_city_hall()
        if self.is_city_hall_upgradable():
            self.recursive_upgrade()
        else:
            self.print("City hall is upgrading..")
    ...
    @get_name
    def dynamique_city_upgrade(self):
    ...
    def upgrade_city(self):
        ...
        self.better_sleep((0.9, 1.2))
        self.enter_hut()
        if self.is_able_to_build():
            self.upgrade_build()
        else:
            while not self.is_able_to_build():
                sleep(1)
            else:
    ...
    def upgrade_build(self):
        co = self.adb.find_img(target='build')
        self.click(co[0] + uniform(10, 60), co[1] + uniform(0, 30))
        self.better_sleep((0.9, 1.2))
        if self.is_in_builder():
            self.click(uniform(256, 310), uniform(450, 570))
            self.better_sleep((0.9, 1.2))
            co = self.adb.find_img(target="validate_build")
            self.click(co[0] + uniform(0, 30), co[1] + uniform(0, 20))
        else:
            co = self.adb.find_img(target="upgrade_build")
            if co is not None:
                self.click(co[0] + uniform(0, 20), co[1] + uniform(0, 30))
