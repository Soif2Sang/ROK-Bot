...
class TroopTraining(Task):
    ...
    def run_upgrade(self):
        ...
        for name in names:
            if (
                co := self.find_img(target=f"{name}_badge_high_view", confidence=0.60)
            ) is None:
            ...
            co = self.validate_co(
                self.find_img(target="upgrade_button_high_view", confidence=0.8)
            )
            if not co:
                continue
    ...
    def run(self):
        ...
        for name in names:
            if self.data[str(self.sel)]["schedules"][str(self.current_profile)][
                ...
                if (
                    co := self.find_img(target=f"{name}_badge", confidence=0.75)
                ) is None:
                    self.print(f"Unable to locate {name}")
                    continue
                if (
                    self.find_img(target=f"building_speedups", confidence=0.8)
                    is not None
                ):
