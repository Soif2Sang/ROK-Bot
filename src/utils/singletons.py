...
import flet as ft
from src.utils.schemas.application_schemas import ApplicationSettingsSchema
from src.utils.schemas.emulator_schemas import EmulatorListSchema
from src.utils.schemas.worker_schemas import WorkerTypeSchema
    ...
    def startEmulator(self, emulator: str):
        ...
        with self.EmulatorLock:
            if emulator_choice == "ld":
                cmd = f'{ss.application_settings.paths.ldplayer.ldconsole} launch --index {tag}'
            else:
                cmd = f'{ss.application_settings.paths.bluestacks.player} --instance {tag}'
