...
from src.utils.functions import get_class, get_name
...
class AlliancePit(Task):
    ...
    def run(self):
        ...
        if not self.is_pit_ready():
            return self.close_windows()

        if not self.click_on_pit():
            return

        self.better_sleep((1.3, 2))

        pit_tap_to_join = self.find_img("pit_tap_to_join")
        if not pit_tap_to_join:
            self.print("You already have a troop gather the alliance pit.")
            return self.close_windows()
        self.click(*pit_tap_to_join)
