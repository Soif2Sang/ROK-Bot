...
from src.utils.bridge.android_debug_bridge_bluestacks import AdbBluestacks
from src.utils.bridge.android_debug_bridge_ld_player import AdbLd
from src.utils.discord.discord_utils import send_discord_message
from src.utils.functions import colorize_name, colorize_output, current_time, get_name, string_to_co, string_to_co_slide
from src.utils.bridge.pc_bridge import PcBridge
from src.utils.singletons import ApiSingleton, EmulatorSingleton, FileSingleton
from src.utils.supabase_auth import SupabaseClient
from src.utils.twocaptcha import TimeoutException, TwoCaptcha
from src.utils.twocaptcha.api import ApiException, NetworkException
...
class Task:
    def __init__(self, tile):
        ...
        if self.tile.__class__.__name__ != "TileWorker":
            if emulator == "bluestacks":
                ...
                self.adb = PcBridge(self.sel, task_reference=self)
    ...
    def free_troop_commander_list(self) -> bool:
        ...
        :return: False if queues are occupied
        """
        cropped_image = self.adb.get_screen()[109:123, 1235:1260]
    ...
    def zoom_out_city(self) -> None:
        ...
        self.script_pause()
        count = 0
        while self.find_img("pc\\loop", confidence=0.8):
            self.script_pause()
            left, top, right, bot = self.adb.get_screen_pos()
            pyautogui.moveTo(left + 1280 / 2 + uniform(-20, 20), top + 720 / 2 + uniform(-20, 20))
    ...
    def close_osiris_popup(self, cv_image=None):
        ...
        if self.find_img(target="osiris_invitation", source=cv_image[: 720 // 2, 1280 // 4 : 1280 - 1280 // 4]):
            self.click(1280 / 2 + uniform(0, 5), 720 / 2 + uniform(0, 5))
            self.better_sleep((3.5, 4.7))
            self.click(960 + uniform(0, 5), 108 + uniform(0, 5))
    ...
    def in_city(self) -> bool:
        ...
        return (
            self.find_img(
                target="pc\\checkpoint_star",
                source=self.adb.get_screen()[:60, :600],
                confidence=0.9,
            )
            is None
        )
    ...
    def close_windows(self, screen=None):
        ...
        while cos := self.adb.find_multiple_img(target="pc\\close_window", source=image[: 720 // 2, 1280 // 2 :]):
            co = cos[-1]
            self.adb.click(co[0] + uniform(3, 9) + 1280 // 2, co[1] + uniform(3, 9))
            self.better_sleep((1.3, 2.8))
            image = self.adb.get_screen()

        while cos := self.adb.find_multiple_img(target="pc\\close_window2", source=image[: 720 // 2, : 1280 // 4], confidence=0.83):
            co = cos[-1]
            self.adb.click(co[0] + uniform(3, 9), co[1] + uniform(3, 9))
            self.better_sleep((1.3, 2.8))
            image = self.adb.get_screen()

        while cos := self.adb.find_multiple_img(target="pc\\close_window3", source=image[: 720 // 2, 1280 // 2 :], confidence=0.83):
            co = cos[-1]
            self.adb.click(co[0] + uniform(3, 9) + 1280 // 2, co[1] + uniform(3, 9))
            self.better_sleep((1.3, 2.8))
            image = self.adb.get_screen()
    ...
    def go_back_to_city(self, deadstop=0):
        ...
        if co := self.find_img(source=image, target="pc\\green_home_button"):
    ...
    def adjusted_leave_city(self, x_click: int, y_click: int) -> None:
        ...
        if self.validate_co((1280 - x_click, 720 - y_click)):
            self.swipe(1280 - x_click, 720 - y_click, 1280 // 2 + uniform(-10, 10), 720 // 2 + uniform(-10, 10))
        else:
            self.little_zoom_from_x_y(x_click, y_click)
