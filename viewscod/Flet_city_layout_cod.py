...
def main(page: ft.Page):
    ...
    def on_tap_update(e: ft.ControlEvent):
        ...
        try:
            ...
            data[str(sel)]["schedules"][str(profile)][page.current_build] = (
                int(e.local_x * 2),
                int(e.local_y * 2),
            )
    ...
    page.column2.controls.extend(
        [
            ft.ElevatedButton(
                text="Set Infantry camp",
                on_click=lambda _: setCurrentBuild("infantry_camp"),
            ),
            ft.ElevatedButton(
                text="Set Cavalry camp",
                on_click=lambda _: setCurrentBuild("cavalry_camp"),
            ),
            ft.ElevatedButton(
...
def main2(sel, profile):
    ...
    def on_tap_update(e: ft.ControlEvent):
        ...
        try:
            print(data[str(sel)]["schedules"][str(profile)])
            data[str(sel)]["schedules"][str(profile)][current_build] = (
                int(e.local_x * 2),
                int(e.local_y * 2),
            )
        except Exception as e:
    ...
    def __init__(self, instance, profile, **kwargs):
        ...
        self.buttons.controls.extend(
            [
                ft.ElevatedButton(
                    text="Set Infantry camp",
                    on_click=lambda _: self.setCurrentBuild("infantry_camp"),
                ),
                ft.ElevatedButton(
                    text="Set Cavalry camp",
                    on_click=lambda _: self.setCurrentBuild("cavalry_camp"),
                ),
                ft.ElevatedButton(
                    text="Set Archer camp",
                    on_click=lambda _: self.setCurrentBuild("archery_camp"),
                ),
                ft.ElevatedButton(
                    text="Set Siege camp",
                    on_click=lambda _: self.setCurrentBuild("siege_camp"),
                ),
                ft.ElevatedButton(
                    text="Set Scout camp",
                    on_click=lambda _: self.setCurrentBuild("scout_camp"),
                ),
                ft.ElevatedButton(
                    text="Set City to transfer",
                    on_click=lambda _: self.setCurrentBuild("research_center"),
                ),
            ]
        )
        for co in [
