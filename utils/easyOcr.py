...
class Reader:
    def __init__(self, is_cuda=False):
        self.reader = easyocr.Reader(['en'], gpu=is_cuda, model_storage_directory=os.path.join('models'), download_enabled=True, verbose= False)

    def __call__(self, img, allowlist=None):
        return self.extract_text(img, allowlist=allowlist)

    def extract_text(self, img, show_text=False, show_confidence=False, allowlist= None):
        result = self.reader.readtext(img, allowlist=allowlist)
