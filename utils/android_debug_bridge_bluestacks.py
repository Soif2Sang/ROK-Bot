...
from utils.functions import accurate_sleep, current_time, get_name, BluestacksConfigParser
...
class AdbBluestacks(Adb):
    def __init__(self, instance: str, host="127.0.0.1", port=5037, task_reference=None):
        super().__init__(instance, host, ss.emulator_settings.emulators[instance].port, task_reference)
    ...
    def wait_boot_complete(self, timeout=100, timedelta=1):
        ...
        while True:
            ...
            try:
                print("try")
                subprocess.run([ss.application_settings.paths.adb, "connect", f"127.0.0.1:{self.port}"])
