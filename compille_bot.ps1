...
$verify_command = "py -3.11 import_verification.py"
...
$new_filename = ".\\auth compiled\\test environnement\\bot_executable_$($current_date)\\bot-$($current_date).exe"

# Create the new directory
New-Item -ItemType Directory -Force -Path ".\\auth compiled\\test environnement\\bot_executable_$($current_date)"

# Move the bot.exe file
Move-Item -Force -Path ".\\bot.exe" -Destination $new_filename

# Copy the resources directory
Copy-Item -Recurse -Force -Path ".\\resources" -Destination ".\\auth compiled\\test environnement\\bot_executable_$($current_date)\\resources"
