...
class GatherRss(Task):
    ...
    def free_troop(self) -> bool:
        ...
        native_text = pytesseract.image_to_string(
            cropped_image3,
    ...
    def select_resource_type(self, place: str) -> tuple[float, float]:
        ...
        if (
            ...
            x, y = uniform(mana_icon[0][0], mana_icon[0][1]), uniform(
                mana_icon[1][0], mana_icon[1][1]
            )
    ...
    def run(self, node_type=None, level_decrease=0):
        ...
        if node_type == "Done":
            ...
            return
        if (
            self.data[str(self.sel)]["schedules"][self.current_profile][node_type]
            == "nothing"
        ):
            return
        ...
        if True:
            ...
            if (
                ...
                .get(f"{node_type}_level")
                - level_decrease
                <= 0
            ):
                node_type = self.next_resource_type(node_type)
                self.print(
                    f"Cannot decrease the current level.. Too low ! next type : {node_type}"
                )
