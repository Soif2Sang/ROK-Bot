...
class GeneralSettings(PageSettings):
    ...
    def init(self):
        ...
        self.add(
            ...
            ft.Switch(label="Logger autoscroll", value=self.data["interface"]["auto_scroll"],
                      on_change=lambda _: self.reverse_keyword("auto_scroll")),
            ft.Switch(label="Limit Logs to 300 (reduce lags)", value=self.data["interface"].get("limit_logs", False),
                      on_change=lambda _: self.reverse_keyword("limit_logs")),
            ft.Switch(label="Enable Discord Notifications", value=self.data["discord"]["enabled"],
