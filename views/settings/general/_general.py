...
class GeneralSettings(PageSettings):
    ...
    def init(self):
        ...
        self.add(
            ft.TextField(label=translate("Custom API key:"), value=self.data[str(self.instance_index)]['API_KEY'],
            ...
            ft.Switch(label=translate("Logger autoscroll"), value=self.data["interface"]["auto_scroll"],
                      on_change=lambda _: self.reverse_keyword("auto_scroll")),
            ft.Switch(label=translate("Limit Logs to 300 (reduce lags)"), value=self.data["interface"].get("limit_logs", False),
                      on_change=lambda _: self.reverse_keyword("limit_logs")),
            ft.Switch(label=translate("Enable Discord Notifications"), value=self.data["discord"]["enabled"],
                      on_change=lambda _: self.reverse_keyword("enabled")),
            ft.TextField(label=translate("Your discord ID"), value=self.data["discord"]["user_id"], on_change=lambda e: self.submit(e, 'user_id', int))
        )
                ...
                ft.Row(
                    controls=[
                        ...
                        ft.OutlinedButton(
                            text=translate("Settings"),
                            icon=ft.icons.SETTINGS,
                            on_click=lambda _: function(self), style=ButtonStyle(shape={
                                ft.MaterialState.DEFAULT: RoundedRectangleBorder(radius=5),
                            }, ),
                        )
                    ],
