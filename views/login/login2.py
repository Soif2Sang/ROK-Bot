...
class LoginScreen(ft.ResponsiveRow):
    def __init__(self, page, *args, **kwargs):
        ...
        self.textfield_username = ft.TextField(label="Username", **textField, value=self.data.get("user", {}).get("username", ""))
        self.textfield_password = ft.TextField(label="Password", **textField, value=self.data.get("user", {}).get("password", ""))
    ...
    def login(self, e):
        ...
        try:
            ...
            if hasattr(self.initial_page, "generate_toast"):
                self.initial_page.generate_toast("Cannot login to RokNet.", "This account is already linked to another computer.")
            sleep(5)
