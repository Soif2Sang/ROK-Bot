...
set nuitka_command=nuitka --clang --mingw64 --onefile --follow-imports --windows-icon-from-ico=.\Item_Gem.ico --remove-output --output-filename=Bot --windows-company-name=Unknown --windows-product-version=1.0 --onefile-tempdir-spec=C:\Users\Default\AppData\Local\Temp\bot_unknown .\app.py

REM Execute the nuitka command
%nuitka_command%

set current_date=%date:~-4%-%date:~3,2%-%date:~0,2%
set new_filename=.\output\test environnement\bot_executable_%current_date%\bot-%current_date%.exe

mkdir ".\output\test environnement\bot_executable_%current_date%"

move .\bot.exe "%new_filename%"
xcopy /E /I /Y ".\resources" ".\output\test environnement\bot_executable_%current_date%\resources"
